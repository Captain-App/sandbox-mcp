  ...
  describe("getSession", () => {
    ...
    it("should return SessionStorageReadError on R2 failure", async () => {
      ...
      if (Exit.isFailure(result)) {
        ...
        if (error._tag === "Fail") {
          ...
          expect((error.error as SessionStorageReadError).sessionId).toBe("test-session");
          expect((error.error as SessionStorageReadError).cause).toContain("R2 get failed");
        }
      }
    });
    ...
    it("should return SessionStorageReadError for invalid JSON", async () => {
      ...
      if (Exit.isFailure(result)) {
        ...
        if (error._tag === "Fail") {
          expect(error.error).toBeInstanceOf(SessionStorageReadError);
          expect((error.error as SessionStorageReadError).cause).toContain("Invalid JSON");
        }
      }
    });
