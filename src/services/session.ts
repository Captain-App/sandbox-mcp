...
function readIndex(
  bucket: R2Bucket,
): Effect.Effect<{ index: SessionIndex; etag?: string }, SessionStorageReadError> {
...
function writeIndex(
  ...
  return Effect.tryPromise({
    try: async () => {
      ...
      const result = await bucket.put(INDEX_KEY, JSON.stringify(index), options);
 ...
 */
export function makeSessionStorageLayer(bucket: R2Bucket): Layer.Layer<SessionStorage> {
