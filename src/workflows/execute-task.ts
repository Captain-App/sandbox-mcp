 ...
 * 1. Configures the sandbox proxy
 * 2. Creates the run record in R2
 * 3. Mounts R2 storage for persistence
 * 4. Restores any previous session state
 * 5. Clones the repository if needed
 * 6. Starts OpenCode inside the sandbox
 * 7. Executes the task via the OpenCode SDK
 * 8. Gets the session title (auto-generated by OpenCode)
 * 9. Backs up session state
 * 10. Completes the run record in R2
 */
export class ExecuteTaskWorkflow extends WorkflowEntrypoint<Env, TaskParams> {
  ...
  async run(event: WorkflowEvent<TaskParams>, step: WorkflowStep): Promise<TaskResult> {
    ...
    try {
      ...
      // Step 2: Create run record in R2
      ...
      // Step 3: Mount R2 storage for workspace persistence (via proxy)
      ...
      // Step 4: Restore OpenCode session state from backup
      ...
      // Step 5: Clone repository if needed (git uses proxy for auth)
      ...
      if (params.repositoryUrl) {
        const cloneResult = await step.do("clone-repository", async () => {
          ...
          const targetDir = await Sandbox.cloneRepository(
            sandbox,
            params.repositoryUrl as string,
            params.branch,
          );
      ...
      // Step 7: Get session title (auto-generated by OpenCode)
      ...
      // Step 8: Backup session state to R2
