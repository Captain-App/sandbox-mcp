...
const RepositoryInfo = Schema.Struct({
	url: Schema.String.pipe(Schema.startsWith(GITHUB_URL_PREFIX), Schema.annotations({ description: 'GitHub repository URL' })),
...
const SessionConfig = Schema.Struct({
	defaultModel: Schema.String.pipe(Schema.annotations({ description: 'Default AI model for OpenCode' })),
});
...
export const SessionMetadata = Schema.Struct({
	...
	lastActivity: Schema.Number.pipe(Schema.annotations({ description: 'Unix timestamp of last activity' })),
